    ...
    let dbItems = (data ?? []).map((i: any) => {
      ...
      return {
        ...
        batches: (i.item_batches ?? []).map((b: any) => ({
          ...
          purchaseRate: Number(b.purchase_price ?? 0),
          salePrice: Number(b.sale_price ?? 0),
          saleRate: Number(b.sale_price ?? 0),
          rate: Number(b.sale_price ?? 0),
...
export async function update(resource: string, id: string, body: any, actor: MutationActor = {}) {
  ...
  if (resource === 'item-batches') {
    ...
    const values: any = {}
    if ('batchNumber' in body) values.batch_number = String(body.batchNumber).trim()
    ...
    if ('mrp' in body) values.mrp = Number(body.mrp || 0)
    if ('costPrice' in body) values.cost_price = Number(body.costPrice || 0)
    if ('purchasePrice' in body) values.purchase_price = Number(body.purchasePrice || 0)
    if ('salePrice' in body) values.sale_price = Number(body.salePrice || 0)
    if ('salesSchemeDeal' in body) values.sales_scheme_deal = Number(body.salesSchemeDeal || 0)
    if ('salesSchemeFree' in body) values.sales_scheme_free = Number(body.salesSchemeFree || 0)
    if ('purchaseSchemeDeal' in body) values.purchase_scheme_deal = Number(body.purchaseSchemeDeal || 0)
    if ('purchaseSchemeFree' in body) values.purchase_scheme_free = Number(body.purchaseSchemeFree || 0)
    ...
    if ('sourceReportValue' in body) values.source_report_value = Number(body.sourceReportValue || 0)
