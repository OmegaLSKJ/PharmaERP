          ...
          <table className="w-full text-xs min-w-[1020px]">
            <thead>
              <tr className="border-b border-border bg-muted/50 text-muted-foreground uppercase text-[11px]">
                ...
                <th className="text-right px-4 py-3 font-medium">Debit / Credit</th>
              ...
              {displayedParties.map((p) => (
                <tr key={p.id} className={cn("table-row-hover", selectedIds.has(p.id) && "bg-indigo-950/20")}>
                  ...
                  <td className="px-4 py-3 text-right font-mono">
                    {(p.totalDebit !== undefined && p.totalDebit > 0) || (p.totalCredit !== undefined && p.totalCredit > 0) ? (
                      <div className="flex flex-col items-end gap-1">
                        <div className="flex items-center justify-end gap-1.5">
                          <span className="text-[9px] font-bold tracking-wider uppercase text-blue-600 dark:text-blue-400 bg-blue-500/10 border border-blue-500/20 px-1 rounded leading-tight">
                            Dr
                          </span>
                          <span className="font-semibold text-foreground">
                            {formatCurrency(p.totalDebit || 0)}
                          </span>
                          {p.debitCount !== undefined && p.debitCount > 0 && (
                        ...
                        <div className="flex items-center justify-end gap-1.5">
                          <span className="text-[9px] font-bold tracking-wider uppercase text-amber-600 dark:text-amber-400 bg-amber-500/10 border border-amber-500/20 px-1 rounded leading-tight">
                            Cr
                          </span>
                          <span className="font-semibold text-foreground">
                            {formatCurrency(p.totalCredit || 0)}
                          </span>
                          {p.creditCount !== undefined && p.creditCount > 0 && (
                            <span className="text-[10px] text-muted-foreground font-normal">
                              ({p.creditCount})
                            </span>
                          )}
