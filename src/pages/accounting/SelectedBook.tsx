...
export default function SelectedBook() {
  ...
  const filtered = allEntries.filter((d) => {
    if (d.vType !== selectedType) return false
  ...
  return (
    <div className="p-6 space-y-5">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">
            View a focused register for any single voucher type
          </p>
        </div>
        <button
      ...
      <div className="bg-slate-900/50 border border-slate-800 rounded-xl overflow-hidden">
        ...
        <table className="w-full text-xs" id="selected-book">
          <thead>
          ...
          <tfoot>
            <tr className="bg-slate-900/80 border-t border-slate-700 text-white font-bold text-xs">
              <td colSpan={4} className="px-4 py-3">Total ({filtered.length} entries)</td>
