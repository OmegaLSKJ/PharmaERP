    ...
    <div className="space-y-5 p-4 sm:p-6">
      ...
      <section className="glass-surface space-y-4 rounded-2xl p-4 sm:p-5">
        ...
        <div className={cn('rounded-2xl border-2 border-dashed p-8 text-center transition sm:p-10', dragging ? 'border-blue-500 bg-blue-500/10' : 'border-border bg-background/30 hover:border-blue-500/60')} onDragOver={(event) => { event.preventDefault(); setDragging(true) }} onDragLeave={() => setDragging(false)} onDrop={onDrop}>
          ...
          <button type="button" onClick={() => inputRef.current?.click()} disabled={reading} className="mt-4 rounded-lg bg-blue-700 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-600 disabled:opacity-50">Choose file</button>
        </div>
        <div className="flex flex-col justify-between gap-3 rounded-xl border border-border bg-background/30 p-3 sm:flex-row sm:items-center"><div><p className="text-sm font-medium">Use the exact column names</p><p className="text-xs text-muted-foreground">Required: {config.required.join(', ')}</p></div><button type="button" onClick={() => downloadCsvTemplate(`${type}-import-template`, [...config.headers])} className="glass-action inline-flex items-center justify-center gap-2 rounded-lg px-3 py-2 text-sm"><Download size={15} />Download CSV template</button></div>
      </section>

      {(file || errors.length > 0) && <section className="glass-surface space-y-4 rounded-2xl p-4 sm:p-5">
        <div className="flex items-start justify-between gap-3"><div><h2 className="font-semibold">{file?.name ?? 'File validation'}</h2><p className="text-xs text-muted-foreground">{rows.length.toLocaleString()} data rows detected</p></div><button type="button" aria-label="Remove selected file" onClick={resetFile} className="glass-action rounded-lg p-2"><X size={16} /></button></div>
        ...
        <div className="flex justify-end"><button type="button" onClick={importRows} disabled={!file || !rows.length || errors.length > 0 || importing} className="inline-flex items-center gap-2 rounded-lg bg-emerald-700 px-4 py-2 text-sm font-semibold text-white hover:bg-emerald-600 disabled:cursor-not-allowed disabled:opacity-50">{importing ? <Loader2 size={16} className="animate-spin" /> : <Upload size={16} />}{importing ? 'Importing…' : `Import ${rows.length.toLocaleString()} rows`}</button></div>
      </section>}
