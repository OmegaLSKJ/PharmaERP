...
const itemQuery = { select: () => ({ eq: () => ({ eq: () => ({ maybeSingle: async () => ({ data: { id: 'item-1', name: 'Sample product', mrp: 100, sale_rate: 80, purchase_rate: 70, manufacturers: { name: 'Maker' }, hsn_codes: { code: '3004', gst_rate: 12 }, salts: { composition: 'Sample composition' } }, error: null }) }) }) }) }
const batchQuery = {
  select: () => ({
    eq: () => ({
...
describe('product detail endpoint', () => {
  ...
  it('requires a verified session and never falls back to mock records', async () => {
    ...
    const response = await GET(new NextRequest('http://localhost/api/v1/product-detail?itemId=item-1'))
  ...
  it('uses live product and batch data with private no-store headers', async () => {
    const response = await GET(new NextRequest('http://localhost/api/v1/product-detail?itemId=item-1&batchId=batch-1'))
